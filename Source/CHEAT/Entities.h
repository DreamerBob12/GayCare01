...
#define m_hPlayerAnimStateServer 0x25D4
	...
	{
		static int offset = GET_NETVAR("DT_CSPlayer", "m_flFriction") - sizeof(Vector);
...
struct StoredNetvars
{
	...
	StoredNetvars() {}
	~StoredNetvars() {}
	StoredNetvars(IClientEntity* pEntity, StoredNetvars *ForcedVars)
	{
		...
		if (ForcedVars)
		{
			...
			eyeyaw = eyeangles.y;
			absangles = pEntity->GetAbsAngles(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
		
			aimpunch = ForcedVars->aimpunch;
			networkorigin = pEntity->GetNetworkOrigin(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
			origin = pEntity->GetOrigin(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
										   //absorigin = *Entity->GetAbsOrigin(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
			velocity = pEntity->GetVelocity(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
		...
		{
			eyeangles = pEntity->GetEyeAngles();
			eyeyaw = eyeangles.y;
			absangles = pEntity->GetAbsAngles(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
		
			aimpunch = pEntity->GetPunch();
			networkorigin = pEntity->GetNetworkOrigin(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
			origin = pEntity->GetOrigin(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
										  //absorigin = *Entity->GetAbsOrigin(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
			velocity = pEntity->GetVelocity(); //Note: not valid from FRAME_NET_POSTDATAUPDATE_START
	     	viewoffset = pEntity->GetViewOffset();
	
			lowerbodyyaw = pEntity->GetLowerBodyYaw();

			simulationtime = pEntity->GetSimulationTime();
		
			flags = pEntity->GetFlags();
	...
	IClientEntity *groundentity; //could possibly crash if entity is gone
								 //bool isducked;
								 //bool isducking;
								 //bool isinduckjump;
								 //int ducktimemsecs;
...
struct StoredLowerBodyYaw
{
	StoredLowerBodyYaw() {}
	~StoredLowerBodyYaw() {}
	StoredLowerBodyYaw(IClientEntity* Entity, StoredNetvars* CurrentNetvars, StoredLowerBodyYaw* PreviousLowerBodyRecord)
	{
