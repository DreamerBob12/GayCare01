...
void CLegitBot::DoTrigger(CUserCmd *pCmd)
{
	...
	if (!Trace.m_pEntity)
		return;
	if (!Trace.m_pEntity->IsAlive())
		return;
	if (Trace.m_pEntity->GetHealth() <= 0 || Trace.m_pEntity->GetHealth() > 100)
		return;
	if (Trace.m_pEntity->IsImmune())
		return;

	if (!Menu::Window.LegitBotTab.TriggerTeammates.GetState()) {
		if (LocalPlayer->GetTeamNum() == Trace.m_pEntity->GetTeamNum())
			return;
	}
	...
	if (Menu::Window.LegitBotTab.TriggerSmokeCheck.GetState())
	{
		...
		if (GoesThroughSmokeFunction(hackManager.pLocal()->GetEyePosition(), Trace.m_pEntity->GetBonePos(6)))
			return;
	}
