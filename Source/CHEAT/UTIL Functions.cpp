...
bool GameUtils::IsZeus(void* weapon)
{
	...
	if (pWeaponClass->m_ClassID == (int)CSGOClassID::CWeaponTaser || pWeaponClass->m_ClassID == (int)CSGOClassID::CWeaponTaser)
		return true;
	else
		return false;
}
...
Vector GetHitboxPosition(IClientEntity* pEntity, int Hitbox)
{
	...
	if (pEntity->SetupBones(matrix, 128, 0x100, 0.f))
	{
		...
		/*	float HighestDamage = 0;
			...
			if (Menu::Window.RageBotTab.TargetMultipoint.GetState() && hackManager.pLocal()->IsAlive()) // Credits Abortion Clinic
			{
				for (auto i = 0; i <= Menu::Window.RageBotTab.TargetPointscale.GetValue() + 1; i++)
				{
